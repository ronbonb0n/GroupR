...
public class NarrativeScript : MonoBehaviour
{
    ...
    private string endNarrative = "Now that I have all the key fragments, time to get out of here!";
    private void Awake()
    {
    ...
    {
        if (GameManager.State == GAME_STATE.DUNGEON_4_COMPLETE) return endNarrative;
        if (dialogueNumber > dialogues.Length - 1) return "-1";

        return dialogues[dialogueNumber];
    }
    public void endNarrativeStart()
    {
        GameObject.Find("ButtonControls").GetComponent<WorldMapCanvasControls>().onNarrationScreen();
    }
